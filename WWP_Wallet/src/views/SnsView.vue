<template>
	<header class="w-full">
		<div class="h-10"></div>
		<div class="flex justify-center items-center">SNS</div>
		<div class="h-5"></div>
	</header>
	<main class="w-full grow overflow-hidden flex flex-col items-center">
		<div class="px-8 w-full overflow-y-auto">
			...
			<div v-for="item in tkList" :key="item.idx" class="flex flex-col justify-center items-center">
				<div @click="moveTk(item.link)"><img :src="item.image" /></div>
				<div class="h-3"></div>
			</div>
			<div class="h-10"></div>
		</div>
	...
	<footer class="w-full g-footer">
		<div class="px-5 flex justify-around items-center" style="position: relative">
			<div style="position: absolute; left: 50%; transform: translate(-31px, -64px)">
			...
			<div @click="showInReady"><img src="@/assets/images/menu_nft_select.svg" /></div>
